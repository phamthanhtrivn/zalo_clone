...
export const FriendItem = ({ item, setFriends }: any) => {
  ...
  const handleShowProfile = (friendId: string) => {
    const fetchUser = async () => {
      try {
        const data = await userService.getProfile(friendId);
        if (data.success) {
        ...
        {item.friends.map((f: any) => (
          ...
          >
            ...
            <div className="relative">
              <button
                onClick={() =>
                  setOpenId(openId == f.friendId ? "" : f.friendId)
                }
              >
              ...
              {openId === f.friendId && (
                <div className="absolute right-0 -mt-0.5 w-48 bg-white border rounded-xl shadow-lg">
                  <div className="p-3 hover:bg-gray-50 cursor-pointer">
                    <button onClick={() => handleShowProfile(f.friendId)}>
                      Xem thông tin
                    </button>
                  </div>
                  <div className="p-3 hover:bg-gray-50 cursor-pointer">
