...
const getFileIconName = (fileName: string): { name: any; color: string } => {
  ...
  if (lower.endsWith(".doc") || lower.endsWith(".docx")) return { name: "document", color: "#3b82f6" };
  if (lower.endsWith(".xls") || lower.endsWith(".xlsx")) return { name: "grid", color: "#22c55e" };
  if (lower.endsWith(".zip") || lower.endsWith(".rar")) return { name: "archive", color: "#f59e0b" };
  if (lower.endsWith(".mp3") || lower.endsWith(".wav")) return { name: "musical-notes", color: "#8b5cf6" };
...
export default function MessageBubble({
  ...
  const mediaFiles = files.filter((f: any) => f.type === "IMAGE" || f.type === "VIDEO");
      ...
      <View style={{ maxWidth: "75%", alignItems: isMe ? "flex-end" : "flex-start" }}>
        ...
        >
            ...
            >
              ...
              <Text numberOfLines={1} style={{ fontSize: 12, color: "#4b5563" }}>
                {message.repliedId.content?.text ||
                  (message.repliedId.content?.files.length > 0 ? message.repliedId.content.files[message.repliedId.content.files.length - 1].fileName : "[Tệp đính kèm]")}
              </Text>
            </TouchableOpacity>
          )}
          ...
          {mediaFiles.length > 0 && (
            ...
            >
              {mediaFiles.map((file: any, index: number) => {
                ...
                const imgSize = mediaFiles.length === 1
                  ? 200
          ...
          {docFiles.map((file: any, index: number) => {
            const { name: iconName, color: iconColor } = getFileIconName(file.fileName);
            return (
              <View
                key={index}
                style={{
          ...
          <Modal visible={previewIndex !== null} transparent animationType="fade">
            <View style={{
              ...
              <TouchableOpacity
                ...
                onPress={() => previewIndex !== null && handleDownload(mediaFiles[previewIndex])}
                disabled={downloading}
              >
                {downloading
