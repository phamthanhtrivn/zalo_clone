...
export default function SearchScreen() {
  ...
  const handleSearchByPhone = async () => {
    ...
    try {
      ...
      ToastAndroid.show(
        (err as any)?.response?.data?.message || "Không thể tìm kiếm người dùng",
        ToastAndroid.SHORT,
      );
    ...
    <Container className="bg-[#f3f4f6]">
      <View className="bg-white">
        <Header
          back
          centerChild={
            <Text className="text-[20px] font-semibold text-black">Thêm bạn</Text>
          }
        />
        ...
        <View className="px-4 pb-6 pt-2 items-center">
          ...
          >
            <Text className="text-white text-[28px] font-semibold">Tri</Text>

            <View className="w-[188px] h-[188px] bg-white rounded-2xl items-center justify-center mt-4">
              <Ionicons name="qr-code-outline" size={148} color="#111827" />
            </View>
      ...
      <View className="bg-white">
        <TouchableOpacity className="h-16 px-4 flex-row items-center border-b border-gray-100">
        ...
        >
          ...
          <Text className="ml-4 text-[18px] text-black">Bạn bè có thể quen</Text>
        </TouchableOpacity>
      </View>
