...
export class MessagesCallService {
  constructor(
    ...
    private readonly conversationModel: Model<Conversation>,
    private readonly transformService: MessagesTransformService,
    private readonly redisService: RedisService, // Đã thay thế ChatGateway bằng RedisService
  ) {}
  ...
  async createCallMessage(callMessageDto: CallMessageDto) {
    ...
    if (populatedMessage) {
      ...
      // Bắn event cập nhật sidebar cho từng member qua Redis
  ...
  async updateCallMessage(updateCallMessageDto: UpdateCallMessageDto) {
    ...
    if (status === CallStatus.MISSED) {
      ...
      await this.redisService.publish(REDIS_CHANNEL_SOCKET_EVENTS, {
        room: conversationId,
        event: 'call_updated',
        data: { messageId, status: CallStatus.MISSED },
      });
    ...
    if (status === CallStatus.REJECTED) {
      ...
      await this.redisService.publish(REDIS_CHANNEL_SOCKET_EVENTS, {
        room: conversationId,
        event: 'call_updated',
        data: { messageId, status: CallStatus.MISSED }, // UI nhận event MISSED để tắt màn hình
      });
    ...
    if (status === CallStatus.BUSY) {
      ...
      await this.redisService.publish(REDIS_CHANNEL_SOCKET_EVENTS, {
        room: conversationId,
        event: 'call_updated',
        data: { messageId, status: CallStatus.BUSY },
      });
    ...
    if (status === CallStatus.ACCEPTED) {
      ...
      await this.redisService.publish(REDIS_CHANNEL_SOCKET_EVENTS, {
        room: conversationId,
        event: 'call_updated',
        data: { messageId, status: CallStatus.ACCEPTED },
      });
