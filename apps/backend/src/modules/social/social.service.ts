...
export class SocialService {
    ...
    async createStory(userId: string, dto: any, files: Express.Multer.File[]) {
        ...
        const mode = (dto.privacyMode || 'friends') as 'friends' | 'include' | 'exclude';
        ...
        const created = await this.postModel.create({
            ...
            storyPrivacy: {
                mode,
                includeUserIds: (dto.includeUserIds || []).map((id: string) => new Types.ObjectId(id)),
                excludeUserIds: (dto.excludeUserIds || []).map((id: string) => new Types.ObjectId(id)),
            },
        });
        ...
        const payload = {
            ...
            story: {
                ...
                mediaUri,
                mediaType: created.content?.media?.[0]?.type || (created.content?.text ? 'TEXT' : 'IMAGE'),
    ...
    async getStories(viewerId: string) {
        ...
        const canView = (story: any) => {
            ...
            const exclude = (story.storyPrivacy?.excludeUserIds || []).map((id: any) =>
                id?.toString?.() || String(id),
            );
        ...
        for (const s of visibleStories) {
            ...
            const mediaUri = s.content?.media?.[0]?.url
                ...
                : '';
            const mediaType = s.content?.media?.[0]?.type || (s.content?.text ? 'TEXT' : 'IMAGE');
            const reactionCounts: Record<string, number> = {};
    ...
    async getStoryViewers(storyId: string, userId: string) {
        ...
        const viewerIds = (story.storyViews || []).map((v: any) => new Types.ObjectId(v.userId));
        ...
        const reactionMap = new Map(
            (story.storyReactions || []).reduce((acc: Map<string, string[]>, reaction: any) => {
    ...
    async reactStory(storyId: string, userId: string, type: string) {
        ...
        if (!allowed.includes(type)) throw new BadRequestException('Invalid reaction type');
        const story = await this.postModel.findOne({
            _id: new Types.ObjectId(storyId),
            postType: 'STORY',
            expiresAt: { $gt: new Date() },
        ...
        if (idx > -1) {
            ...
            list.push({ userId: new Types.ObjectId(userId), type, createdAt: new Date() });
        }
    ...
    async replyStory(storyId: string, userId: string, content: string) {
        ...
        story.storyReplies = [
            ...(story.storyReplies || []),
            { userId: new Types.ObjectId(userId), content: text, createdAt: new Date() } as any,
        ];
        await story.save();
        ...
        const [sender, author] = await Promise.all([
            this.postModel.db
            .collection('users')
            .findOne({ _id: new Types.ObjectId(userId) }, { projection: { profile: 1 } }),
            this.postModel.db
            .collection('users')
            .findOne({ _id: new Types.ObjectId(story.authorId) }, { projection: { profile: 1 } }),
        ]);

        const conversation: any = await this.conversationsService.getOrCreateDirectConversation(
            userId,
            String(story.authorId),
        );

        const previewImageKey = story.content?.media?.[0]?.url || '';
        const previewText = story.content?.text?.trim()
            || (story.content?.media?.[0]?.type === 'VIDEO' ? 'Da gui mot story video' : 'Da gui mot story');
    ...
    async getStoryMusicSuggestions(q: string) {
        ...
        const fallbackTracks = Array.isArray(fallback?.tracks) ? fallback.tracks : [];
